...
def _actively_learn(problem, train_ixs, obs_labels, oracle, **kwargs):
    """
    ...
    query_strat = kwargs.get("query_strat", uncertainty_sampling)
    ...
    for i in tqdm(range(num_queries)):
        ...
        selected_ixs = query_strat(problem, train_ixs, obs_labels, unlabeled_ixs, batch_size, budget=num_queries - i,
                                   **kwargs)
...
def actively_learn_holdout(problem, train_ixs, obs_labels, oracle, holdout_points, holdout_iter, **kwargs):
    selector = kwargs.get("selector", identity_selector)
    query_strat = kwargs.get("query_strat", uncertainty_sampling)
    callback = kwargs.get("callback")
    ...
    for i in tqdm(range(num_queries)):
        if i == holdout_iter:
            problem['points'] = np.vstack([problem['points'], holdout_points])
